...
def my_euclidean_dist(X_test, X_train):
    ...
    import math
    #Both of these arrays will cold the resukts fronm the first 2 for loops
    a = np.zeros(num_test) #a is an array of size num_test
    b = np.zeros(num_train) #b is an array of

    #Looping through X_tests
    for i in range(num_test):
        for j in range( X_train.shape[1]):
            a[i] += pow(X_test[i][j],2) 

    #Looping through X_trains
    for i in range(num_train):
        for j in range( X_train.shape[1]):
            b[i] += pow(X_train[i][j],2) 
...
def find_k_neighbors(dists, Y_train, k):
    ...
    for i in range(0,dists.shape[0]):
        ...
        neighbors[i] = [Y_train[idx[j]] for j in range(0, k)]
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return neighbors
...
def knn_predict(X_test, X_train, Y_train, k):
    ...
    for i in range(0,X_test.shape[0]):
        dummy_unique,num = (np.unique(neighbors[i],return_counts= True))
        idx = (np.argmax(num))
        Y_pred[i] = dummy_unique[idx]
...
def compute_error_rate(ypred, ytrue):
    ...
    for i in range(0, len(ypred)):
        error += (ypred[i] != ytrue[i])
    error_rate = (error / len(ypred)) * 100
...
def split_nfold(num_examples, n):
    ...
    for i in range(0,n):
        ...
        train_set = [k for i, k in enumerate(idx) if k not in idx[start:end]]
        train_sets.append(train_set)
...
def problem2():
    ...
    print(f'#tr = {trial_size}, error rate = {error_rates[t]}%')
    ...
    #call the 'cross_validation' function to get the error rate for the current trial (of fold number)
    for t in range(trials):
...
def problem3():
    ...
    #plt.title('err_ks versus list_ks') 
    # naming the x axis 
    #plt.xlabel('x - axis') 
    # naming the y axis 
    #plt.ylabel('y - axis') 
    ...
    # Print the error rate with a line like 'The test error is x.y%'
    print('The test error rate is ', err_test, '%')
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
